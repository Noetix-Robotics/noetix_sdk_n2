...
bool updateStateEstimation() {
        ...
        std::array<MotorState, 18> joint_state = latest_hw_status.motor_data;
        ...
        NingImuData imudata = latest_hw_status.imu_data;
...
void handleLieMode() {
        // MotorCmd motorcmd;
        std::array<MotorCmd, 18> motorcmd;
        const std::array<MotorState, 18> ms = latest_hw_status.motor_data;
        if (standPercent <= 1) {
                for (int j = 0; j < 18; j++) {
...
bool handleWalkMode() {
        ...
        for (int i = 0; i < 8; i++) {
                ...
                std::string partName = jointNames[j];
                scalar_t pos_des;
                const std::array<MotorState, 18> ms =
